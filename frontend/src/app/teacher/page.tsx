...
export default function TeacherDashboard() {
  ...
  return (
    <div className="space-y-8 w-full">
      {/* Top Banner Header */}
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 bg-slate-900 bg-gradient-to-r from-slate-900 via-purple-950 to-indigo-950 p-8 rounded-3xl border border-purple-800/50 shadow-xl">
        <div>
          <span className="text-xs font-extrabold uppercase tracking-wider text-purple-300">Educator Workspace</span>
          <h1 className="text-3xl font-black text-white mt-1">Welcome back, {user?.firstName}!</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className="p-6 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">My Classes & Subjects</span>
            <div className="w-10 h-10 rounded-xl bg-purple-500/10 text-purple-600 dark:text-purple-400 flex items-center justify-center font-bold">
              <School className="w-5 h-5" />
            </div>
          </div>
          <p className="text-3xl font-black text-slate-900 dark:text-white mt-4">{classSubjects.length}</p>
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">Assigned courses across school</p>
        </div>

        <div className="p-6 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Published Assignments</span>
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">Active coursework visible to students</p>
        </div>

        <div className="p-6 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Draft Assignments</span>
            <div className="w-10 h-10 rounded-xl bg-amber-500/10 text-amber-600 dark:text-amber-400 flex items-center justify-center font-bold">
              <FileText className="w-5 h-5" />
            </div>
          </div>
          <p className="text-3xl font-black text-amber-600 dark:text-amber-400 mt-4">{draftCount}</p>
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">Unpublished work in progress</p>
        </div>

        <div className="p-6 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Total Submissions</span>
            <div className="w-10 h-10 rounded-xl bg-blue-500/10 text-blue-600 dark:text-blue-400 flex items-center justify-center font-bold">
              <Inbox className="w-5 h-5" />
            </div>
          </div>
          <p className="text-3xl font-black text-blue-600 dark:text-blue-400 mt-4">{totalSubmissions}</p>
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">Student responses received</p>
        </div>
      </div>
              ...
              <div className="divide-y divide-slate-200 dark:divide-slate-800">
                {assignments.slice(0, 5).map((item) => (
                  <div key={item.id} className="p-5 flex items-center justify-between hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors">
                    ...
                    <div className="flex items-center space-x-3 shrink-0">
                      <span className="text-xs text-slate-400 font-medium">{item.maxScore} Max Marks</span>
                      <Link
          ...
          {classSubjects.length === 0 ? (
            ...
            <div className="space-y-3">
              {classSubjects.map((cs) => (
                <div key={cs.id} className="p-4 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800/80 shadow-sm flex items-center justify-between">
                  <div>
                    <p className="font-bold text-slate-900 dark:text-white text-sm">{cs.className} ({cs.classCode})</p>
                    <p className="text-xs text-purple-500 font-medium">{cs.subjectName} ({cs.subjectCode})</p>
                  </div>
