...
export default function StudentDashboard() {
  ...
  return (
    <div className="space-y-8 w-full">
      {/* Welcome Banner */}
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 bg-slate-900 bg-gradient-to-r from-slate-900 via-pink-950 to-purple-950 p-8 rounded-3xl border border-pink-800/50 shadow-md">
        <div>
        ...
        <div className="flex items-center space-x-3 shrink-0">
          <Link
            ...
            className="px-5 py-3 rounded-xl bg-pink-600 hover:bg-pink-500 text-white font-semibold text-sm shadow-md shadow-pink-600/30 transition-all inline-flex items-center space-x-2"
          >
                  ...
                  <div key={item.id} className="p-5 flex items-center justify-between hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors">
                    <div className="space-y-1">
                      <div className="flex items-center space-x-2">
                        ...
                        {item.hasSubmitted ? (
                          <span className="px-2.5 py-1 rounded-md bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20 text-xs font-semibold">Submitted</span>
                        ) : item.isOverdue ? (
                          <span className="px-2.5 py-1 rounded-md bg-red-500/10 text-red-600 dark:text-red-400 border border-red-500/20 text-xs font-semibold">Overdue</span>
                        ) : (
                          <span className="px-2.5 py-1 rounded-md bg-amber-500/10 text-amber-600 dark:text-amber-400 border border-amber-500/20 text-xs font-semibold">Pending</span>
                        )}
                      </div>
        ...
        <div className="space-y-4">
          ...
          {classes.length === 0 ? (
            ...
            <div className="space-y-3">
              {classes.map((c) => (
                <div key={c.classId} className="p-4 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800/80 shadow-md">
