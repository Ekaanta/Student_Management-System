...
export default function HomePage() {
  return (
    <main className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 flex flex-col justify-between selection:bg-indigo-500 selection:text-white relative transition-colors duration-300 w-full max-w-full overflow-x-hidden">
      {/* Header Bar */}
      <header className="w-full border-b border-slate-200 dark:border-slate-800/80 px-4 sm:px-6 py-3 sm:py-4 backdrop-blur-xl bg-white/70 dark:bg-slate-950/70 sticky top-0 z-50 transition-colors duration-300">
        <div className="max-w-7xl mx-auto flex items-center justify-between gap-2 w-full">
          <div className="flex items-center space-x-2 sm:space-x-3 shrink min-w-0">
            ...
            <span className="text-xs xs:text-sm sm:text-xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-slate-900 via-slate-800 to-slate-600 dark:from-white dark:via-slate-100 dark:to-slate-400 truncate">
              OnnoRokom Projukti
            </span>
          </div>
      ...
      <section className="w-full max-w-7xl mx-auto px-4 sm:px-6 py-10 sm:py-20 text-center flex-1 flex flex-col items-center justify-center">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-3 sm:gap-4 w-full max-w-md mx-auto">
          ...
          <Link
            href="/register"
            className="w-full sm:w-auto px-8 py-4 rounded-xl text-base font-semibold text-slate-700 dark:text-slate-300 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 hover:bg-slate-100 dark:hover:bg-slate-800 hover:text-slate-900 dark:hover:text-white transition-all text-center"
          >
            Register Account
          </Link>
        </div>
      ...
      <section className="w-full max-w-7xl mx-auto px-4 sm:px-6 py-8 sm:py-12 border-t border-slate-200 dark:border-slate-800/50 transition-colors duration-300">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 sm:gap-8 w-full justify-items-center items-stretch">
          {/* Admin Role */}
          <div className="w-full max-w-md md:max-w-none mx-auto p-6 sm:p-8 rounded-2xl bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800/80 hover:border-indigo-500/50 transition-all duration-300 group flex flex-col justify-between shadow-sm hover:shadow-md box-border">
            <div>
          ...
          {/* Teacher Role */}
          <div className="w-full max-w-md md:max-w-none mx-auto p-6 sm:p-8 rounded-2xl bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800/80 hover:border-purple-500/50 transition-all duration-300 group flex flex-col justify-between shadow-sm hover:shadow-md box-border">
            <div>
          ...
          {/* Student Role */}
          <div className="w-full max-w-md md:max-w-none mx-auto p-6 sm:p-8 rounded-2xl bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800/80 hover:border-pink-500/50 transition-all duration-300 group flex flex-col justify-between shadow-sm hover:shadow-md box-border">
            <div>
      ...
      {/* Footer */}
      <footer className="w-full border-t border-slate-200 dark:border-slate-800/80 py-6 sm:py-8 px-4 text-center text-slate-500 dark:text-slate-500 text-xs sm:text-sm transition-colors duration-300">
